...
from model.kernels.linear import linear, linear_tuned
from tests.conftest import TOLERANCES

TOL = TOLERANCES["linear"]
VARIANTS = [linear, linear_tuned]
...
def test_matches_torch(device, fn, batch, k, n):
    x = torch.randn(batch, 64, k, device=device)
    w = torch.randn(n, k, device=device) * 0.05
    b = torch.randn(n, device=device)
    torch.testing.assert_close(fn(x, w, b), F.linear(x, w, b), **TOL)


@pytest.mark.parametrize("fn", VARIANTS)
def test_without_bias(device, fn):
    x = torch.randn(4, 64, 192, device=device)
    w = torch.randn(192, 192, device=device) * 0.05
    torch.testing.assert_close(fn(x, w, None), F.linear(x, w, None), **TOL)


@pytest.mark.parametrize("fn", VARIANTS)
def test_non_contiguous_input(device, fn):
...
@pytest.mark.parametrize("fn", VARIANTS)
def test_non_power_of_two_k_dimension(device, fn):
