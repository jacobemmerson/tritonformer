...
TOL = TOLERANCES["mlp"]
VARIANTS = [mlp_composed, mlp_fused, mlp_fused_lowreg, mlp_fused_blockh]
...
def test_fused_lowreg_matches_composed_exactly_enough(device):
    """Same tight cross-check as test_fused_matches_composed_exactly_enough,
    for the register-reduced variant (Experiment 1, The Flip)."""
    x = torch.randn(8, 64, 192, device=device)
    w1 = torch.randn(768, 192, device=device) * 0.05
    b1 = torch.randn(768, device=device)
    w2 = torch.randn(192, 768, device=device) * 0.05
    b2 = torch.randn(192, device=device)
    torch.testing.assert_close(mlp_fused_lowreg(x, w1, b1, w2, b2),
                               mlp_composed(x, w1, b1, w2, b2),
                               rtol=1e-5, atol=1e-5)


def test_fused_blockh_matches_composed_exactly_enough(device):
    """Same tight cross-check as test_fused_matches_composed_exactly_enough,
    for the BLOCK_H-reduced variant (Experiment 1b, the real flip test)."""
    x = torch.randn(8, 64, 192, device=device)
    w1 = torch.randn(768, 192, device=device) * 0.05
    b1 = torch.randn(768, device=device)
    w2 = torch.randn(192, 768, device=device) * 0.05
    b2 = torch.randn(192, device=device)
    torch.testing.assert_close(mlp_fused_blockh(x, w1, b1, w2, b2),
                               mlp_composed(x, w1, b1, w2, b2),
                               rtol=1e-5, atol=1e-5)
